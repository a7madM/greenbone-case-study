...
func TestAssignComputer(t *testing.T) {
	...
	req := httptest.NewRequest("PUT", fmt.Sprintf("/api/computers/%d/assign/EM2", computer.ID), nil)
	...
	assert.Nil(t, err)
	assert.Equal(t, "EM2", updatedComputer.EmployeeAbbreviation)
}
func TestAssignComputerNotFound(t *testing.T) {
	app := setupTestApp()
	fakeID := 9999
	req := httptest.NewRequest("PUT", fmt.Sprintf("/api/computers/%d/assign/EM3", fakeID), nil)
	resp, err := app.Test(req)
	assert.Nil(t, err)
